...
namespace {
	template<class T>
	void delete_vector(std::vector<T*>& objs) {
		for (T* obj: objs) {
			delete obj;
		}
		objs.clear();
...
void QtUIFactory::clear_data() {
	...
	delete_vector(boxes);
	delete_vector(full_boxes);
	delete_vector(ships);
	delete_vector(enemies);
	delete_vector(flying_enemies);
	delete_vector(jumping_enemies);
	delete_vector(moneys);
}
